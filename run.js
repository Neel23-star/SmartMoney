...
// Then: cd backend && npm install && node server.js
// Then (new terminal): cd frontend && npm install && npm run dev
...
console.log('\n🚀 AI Approval Assistant - Full Setup\n');
...
write('backend/mockAI.js', `function analyzeRequest({ employee_name, employee_id, request_type, department, description, amount, attachment_name }) {
  const warnings = [];
...
write('backend/database.js', `const Database = require('better-sqlite3');
const path = require('path');
...
write('backend/routes/requests.js', `const express = require('express');
...
write('backend/routes/dashboard.js', `const express = require('express');
const router = express.Router();
const { db } = require('../database');
...
write('README.md', `# AI Approval Assistant
...
console.log('  Terminal 1: cd backend && npm install && node server.js');
console.log('  Terminal 2: cd frontend && npm install && npm run dev');
